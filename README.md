...
- **Resume-capture without exposing `$HOME`.** The container's `HOME` is set to a dedicated per-session host directory (`~/.varda/sessions/{session_id}`) that is bind-mounted read-write — the host's real `$HOME` is never mounted, so credentials stay out of the container. The agent writes its own session store (claude/copilot/codex) under that HOME, and Varda reads it back from the host after the run to produce a working `resume_command`. **Caveat (VM-backed docker):** this round-trip requires `~/.varda/sessions` to be within the VM's shared tree. On a VM-backed daemon that shares only a narrow root (e.g. a Colima profile mounting only `~/dev`), `~/.varda` is *not* visible, so the per-session HOME binds as an empty in-VM stub and resume-capture silently no-ops. Ensure the session-store root is VM-visible (mount `~/.varda` in the VM, or relocate the session root under a shared path).
