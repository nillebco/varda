...
- **Egress modes are explicit.** With no `egress` hosts the container gets `--network none` and is fully offline; this is strict/offline for every provider. Docker cannot portably enforce a real IP-level egress firewall for a non-empty allow-list, so the default `egress_mode = "strict"` rejects Docker non-empty egress before launch. To keep the legacy Docker behavior, set `egress_mode = "dns-pin"` explicitly: Varda attaches the container to the bridge network, disables ambient DNS (`--dns 0.0.0.0`), and pins only the allow-listed hostnames via `--add-host`. DNS-pin blocks non-allow-listed hostnames, but an agent that already knows an IP can still make direct-IP connections; it is not clawk/microsandbox-equivalent firewalling.
...
| **G2** network | **strict, firewall-enforced egress to LLM endpoints only** — every `egress` host must be in the fixed `RESIDENT_EGRESS_ALLOWLIST` (`api.anthropic.com`, `api.openai.com`, `chatgpt.com`, `auth.openai.com`; an empty `egress` ⇒ `--network none` also passes). A non-empty resident allow-list requires a provider that can prove direct-IP bypass is closed (`microsandbox`/`clawk` semantics). Docker DNS-pin mode is refused for residents. | the sandbox declares any egress host outside the LLM allowlist, uses `egress_mode = "dns-pin"`, or uses Docker with non-empty strict egress |
