...
> **Status:** the policy engine, live broker, Unix-socket MCP transport, concrete sibling-task launcher, and `[orchestration]` config surface are implemented and covered by unit tests. **Remaining:** a docker-backed negative-isolation integration test (`--ignored`) that exercises a real sandboxed master end-to-end and asserts no docker socket / no `~/.varda` in the guest.
