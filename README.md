...
| **G2** no push cred | the resident identity carries **no `git push` credential** | `forward_ssh_agent = true`, or a credential targets a push channel (`GITHUB_TOKEN`, `SSH_AUTH_SOCK`, a `.ssh/` key, `.git-credentials`, `.netrc`, …) |
